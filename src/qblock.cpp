...
Block::Block(const std::shared_ptr<const Payload> &pyl) : payload_(pyl){};
...
Block::Block(const QJsonValue &val)
    : protocol_version(val.toObject()["protocolVersion"].toInt()), nonce_(val.toObject()["nonce"].toInteger()),
      payload_(Payload::from_<const QJsonValue>(val))
{
    const auto arr = val.toObject()["outputs"].toArray();
    for (const auto &v : arr)
        parents_.push_back(Block_ID(v));
}
...
QJsonObject Block::get_Json(void) const
{
    ...
    for (const auto &v : parents_)
        parentarr.push_back(v.toHexString());
