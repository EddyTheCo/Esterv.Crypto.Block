...
QJsonObject Essence::get_Json(void) const
{
    return QJsonObject();
};
...
std::shared_ptr<const Essence> Essence::Transaction(const quint64 &network_id_m, const pvector<const Input> &inputs_m,
                                                    const c_array &inputs_commitment_m,
                                                    const pvector<const Output> &outputs_m,
                                                    const std::shared_ptr<const Payload> &payload_m)
{
    return std::shared_ptr<Essence>(
        new Transaction_Essence(network_id_m, inputs_m, inputs_commitment_m, outputs_m, payload_m));
}
...
Transaction_Essence::Transaction_Essence(const QJsonValue &val)
    : Transaction_Essence(val.toObject()["networkId"].toInt(), get_Tvec<Input>(val.toObject()["inputs"].toArray()),
...
QJsonObject Transaction_Essence::get_Json(void) const
{
    ...
    QJsonArray inarr;
    for (const auto &v : inputs_)
        inarr.push_back(v->get_Json());
    var.insert("inputs", inarr);

    QJsonArray outarr;
    for (const auto &v : outputs_)
        outarr.push_back(v->get_Json());
