...
class Unlock_Condition
{
  public:
    ...
    {
        Address_typ = 0,
        Storage_Deposit_Return_typ = 1,
        Timelock_typ = 2,
        Expiration_typ = 3,
        State_Controller_Address_typ = 4,
        Governor_Address_typ = 5,
        Immutable_Alias_Address_typ = 6
    };
    ...
    static std::shared_ptr<const Unlock_Condition> Storage_Deposit_Return(
    ...
    static std::shared_ptr<const Unlock_Condition> Expiration(
        const quint32 &unix_time_m, const std::shared_ptr<const class Address> &return_address_m);
    static std::shared_ptr<const Unlock_Condition> State_Controller_Address(
        const std::shared_ptr<const class Address> &address_m);
    static std::shared_ptr<const Unlock_Condition> Governor_Address(
        const std::shared_ptr<const class Address> &address_m);
    static std::shared_ptr<const Unlock_Condition> Immutable_Alias_Address(
        const std::shared_ptr<const class Address> &address_m);
...
{
  public:
    Storage_Deposit_Return_Unlock_Condition(const std::shared_ptr<const class Address> &return_address_m,
...
{
  public:
    Expiration_Unlock_Condition(const quint32 &unix_time_m,
